...
namespace CupcakeMvvM.Behaviors
{
    ...
    /// 
    /// source: https://github.com/BellaCode/MVVM
    /// 
    /// </remarks>
    public class RegisterAppShortcutKeysBehavior : Behavior<FrameworkElement>
    {
        ...
        public static readonly DependencyProperty InputBindingsProperty = DependencyProperty.Register("InputBindings",
            typeof(InputBindingCollection), typeof(RegisterAppShortcutKeysBehavior),
            new FrameworkPropertyMetadata(null, FrameworkPropertyMetadataOptions.Inherits, OnInputBiningsChanged));

        private static void OnInputBiningsChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            var frameworkElement = d as FrameworkElement;

            if (frameworkElement != null)
            {
                var inputBindings = (InputBindingCollection)e.NewValue;
                if (inputBindings != null)
                {
                    RegisterBindings(inputBindings, frameworkElement);
                }
                //RegisterAssociatedWindow(this.frameworkElement);
                if (!RegisterAssociatedWindow(frameworkElement))
                {
        ...
        protected override void OnAttached()
        {
            ...
            if (this.frameworkElement != null)
            {
                ...
                //RegisterAssociatedWindow(this.frameworkElement);
                if (!RegisterAssociatedWindow(this.frameworkElement))
                {
                    this.frameworkElement.Loaded += new RoutedEventHandler(CommandTarget_Loaded);
                }
            }
        }
        ...
        #endregion Registration Helper Methods
